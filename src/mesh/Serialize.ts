/**
 * Serialize graph data {regions, _s_start_r, numBoundaryRegions, numSolidSides, _s_opposite_s}
 ...
 * regions: Float32Array[2*numRegions] -- the x,y positions
...
function serializeMesh({numBoundaryRegions, numSolidSides, _r_vertex, _s_start_r, _s_opposite_s}
                           : { numBoundaryRegions: any, numSolidSides: any, _r_vertex: any, _s_start_r: any, _s_opposite_s: any }) {
    ...
    for (let i = 0; i < _r_vertex.length; i++) {
        dv.setFloat32(p, _r_vertex[i][0]);
        p += 4;
        dv.setFloat32(p, _r_vertex[i][1]);
        p += 4;
    }
